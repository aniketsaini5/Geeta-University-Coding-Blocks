...
int main() {
    ...
    for (int i = 0; i < Q; ++i) {
		// query type 1 fro student 2 for instructor
        int queryType;
        cin >> queryType;

        if (queryType == 1) {
            if (moduleStack.empty()) {
                cout << "No Code" << endl; // return in case of student  if stack is empty 
            } else {
                cout << moduleStack.top() << endl;
                moduleStack.pop();  // give a module to student 
            }
        } else if (queryType == 2) {  // inst query
            int cost;
            cin >> cost;
