...
int main() {
    ...
    for (int i = 0; i < N; ++i) {
        while (callingOrder.front() != idealOrder[i]) {
            int process = callingOrder.front();
            callingOrder.pop();
            callingOrder.push(process);
            time++; // Time for rotating the calling order
        }
        callingOrder.pop(); // Process the correct process
        time++; // Time for executing the process
    }
